...
__global__ void matMulTiledKernel(const float* __restrict__ A,
                                  ...
                                  int N) 
{
    // TODO: Declare shared memory A_s and B_s
    

    // TODO: Calculate the global coordinates corresponding to this thread
    int row = 
    int col = 
    ...
    for (int tile = 0; tile < (N / TILE_DIM); ++tile) {
        ...
        A_s[threadIdx.y][threadIdx.x] = A['''TODO'''];
        B_s[threadIdx.y][threadIdx.x] = B['''TODO'''];

        // Wait for all threads to finish loading
        __syncthreads();

        // TODO: Perform computation in shared memory
...
int main()
{
    ...
    // Note that N should be divisible by TILE_DIM
    const int N = 512;  
