...
__global__ void compute_spillage_kernel(int rows, int columns, const float *ground, const int *water_level,
    ...
    if (water_i <= 0) {
        // CUDA fast path: skip dry cells entirely (seq reaches the same result via nested ifs).
        return;
    }
    ...
    // Cache neighbor diffs once so the second pass can reuse them (seq recomputes neighbor heights).
...
#pragma unroll
            for (int dir = 0; dir < CONTIGUOUS_CELLS; dir++) {
                ...
                if (new_row < 0 || new_row >= rows || new_col < 0 || new_col >= columns) {
                    // Border water-loss accumulation is shared across threads, so it must be atomic on GPU.
                    ...
                    int nidx = idx2d(new_row, new_col, columns);
                    // Keep seq layout: write incoming flow at destination cell + direction slot.
...
__global__ void rainfall_per_cloud_kernel(int rows, int columns,
                                          float cx, float cy, float radius2, float inv_radius,
                                          float cintensity, float sqrt_intensity,
    ...
    if (dist2 < radius2) {
        ...
        float rain = ex_factor * fmaxf(0.0f, cintensity - distance * inv_radius * sqrt_intensity);
...
__global__ void apply_spillage_kernel(int rows, int columns, int *water_level, float *spillage_flag,
    ...
    extern __shared__ unsigned int s_block_max[];

    unsigned int local_max_bits = 0U;
    ...
    if (row < rows && col < columns) {
        ...
        if (spillage_flag[idx] == 1.0f) {
            ...
            water -= FIXED(spill);
            // For non-negative spills, uint bit ordering matches float ordering for max reduction.
            local_max_bits = __float_as_uint(spill);
        }
...
#pragma unroll
        for (int dir = 0; dir < CONTIGUOUS_CELLS; dir++) {
            ...
            // Fold seq's separate end-of-iteration reset loop into this consumption pass.
    ...
    s_block_max[tid] = local_max_bits;
    __syncthreads();

    // Parallel reduction to find the maximum spillage level in this block.
    for (int stride = nthreads >> 1; stride > 0; stride >>= 1) {
        if (tid < stride) {
            unsigned int other = s_block_max[tid + stride];
            if (other > s_block_max[tid]) {
                s_block_max[tid] = other;
            }
        }
        __syncthreads();
    }

    if (tid == 0 && s_block_max[0] > 0U) {
        atomicMax(max_spillage_bits, s_block_max[0]);
    }
}
...
extern "C" void do_compute(struct parameters *p, struct results *r) {
    ...
    size_t apply_shared_bytes = (size_t)block.x * (size_t)block.y * sizeof(unsigned int);
    ...
    for (*minute = 0; *minute < p->num_minutes && max_spillage_iter > p->threshold; (*minute)++) {
        ...
        for (int cloud = 0; cloud < p->num_clouds; cloud++) {
            Cloud_t c_cloud = p->clouds[cloud];
            if (c_cloud.radius <= 0.0f || c_cloud.intensity <= 0.0f) {
                continue;
            }
            ...
            float inv_radius = 1.0f / c_cloud.radius;
            float sqrt_intensity = sqrtf(c_cloud.intensity);
            ...
            rainfall_per_cloud_kernel<<<rain_grid, rain_block>>>(
                rows, columns, c_cloud.x, c_cloud.y, radius2, inv_radius,
                c_cloud.intensity, sqrt_intensity,
        ...
        apply_spillage_kernel<<<grid, block, apply_shared_bytes>>>(rows, columns, d_water_level, d_spillage_flag,
