...
__global__ void rainfall_kernel(int rows, int columns, int num_clouds, float ex_factor,
                                const Cloud_t *clouds, int *water_level, unsigned long long *total_rain) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (int k = 0; k < num_clouds; k++) {
        ...
        // Replicate seq's fractional row_pos = row_start + j, where j = row - (int)row_start
        float rs_raw = c.y - c.radius;
        float re_raw = c.y + c.radius;
        float cs_raw = c.x - c.radius;
        float ce_raw = c.x + c.radius;
        float rs = (rs_raw < 0.0f ? 0.0f : rs_raw) * (float)rows / (float)SCENARIO_SIZE;
        float re = (re_raw > (float)SCENARIO_SIZE ? (float)SCENARIO_SIZE : re_raw) * (float)rows / (float)SCENARIO_SIZE;
        float cs = (cs_raw < 0.0f ? 0.0f : cs_raw) * (float)columns / (float)SCENARIO_SIZE;
        float ce = (ce_raw > (float)SCENARIO_SIZE ? (float)SCENARIO_SIZE : ce_raw) * (float)columns / (float)SCENARIO_SIZE;

        int j_row = row - (int)rs;
        int j_col = col - (int)cs;
        if (j_row < 0 || j_col < 0) continue;
        float row_pos = rs + (float)j_row;
        float col_pos = cs + (float)j_col;
        if (row_pos >= re || col_pos >= ce) continue;
        ...
        if (distance < (double)c.radius) {
            double rain_d = (double)ex_factor *
                fmax(0.0, (double)c.intensity - distance / (double)c.radius * sqrt((double)c.intensity));
            float rain = (float)rain_d;
            float meters_per_minute = rain / 1000.0f / 60.0f;
            add += (int)(meters_per_minute * (float)PRECISION);
        }
    }

    if (add != 0) {
        water_level[row * columns + col] += add;
        atomicAdd(total_rain, (unsigned long long)add);
    }
}
...
extern "C" void do_compute(struct parameters *p, struct results *r) {
    ...
    for (*minute = 0; *minute < p->num_minutes && max_spillage_iter > p->threshold; (*minute)++) {
        ...
        /* Step 1.2: Rainfall (GPU) — single kernel, all clouds, one thread per cell */
        CUDA_CHECK_FUNCTION(cudaMemcpyAsync(d_clouds, p->clouds,
                                            sizeof(Cloud_t) * (size_t)p->num_clouds,
                                            cudaMemcpyHostToDevice));
        rainfall_kernel<<<grid, block>>>(rows, columns, p->num_clouds, p->ex_factor,
                                         d_clouds, d_water_level, d_total_rain);
        CUDA_CHECK_KERNEL();
